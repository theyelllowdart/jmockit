...
public final class AnimationManagerTest
{
   ...
   @Test
   public void recreateImageForContainerOfSizeZero(@Mocked final JComponent container)
   {
      // recreateImage() is called by the constructor.
      new AnimationManager(container);

      new Verifications() {{ onInstance(container).createImage(anyInt, anyInt); times = 0; }};
   }

   @Test
   public void recreateImageForContainerOfSizeNotZeroAndBackgroundStillUndefined(@Mocked final JComponent container)
   {
   ...
   public void recreateImageForContainerOfSizeNotZeroAndBackgroundAlreadyDefined(
      @Mocked final JComponent container, @Mocked final BufferedImage transitionImageBG)
   {
      new Expectations() {{
         int cw = 100;
         int ch = 80;
   ...
   public void setupStart()
   {
      ...
      new Expectations(manager) {{ manager.addStart(component); }};
      ...
      @Mocked("(ComponentState, boolean)") final AnimationState animationState)
   {
      ...
      new Expectations() {{ animationState.setEnd(new ComponentState(component)); }};
